...
class ReportController extends BaseController
{
    ...
    public function ProfitAndLoss(request $request)
    {
        ...
        $item['expenses_money_sent'] = Expense::where('expense_category_id', '=', '3')->whereBetween('date', array($request->from, $request->to))
            ->select(
                DB::raw('SUM(amount) AS sum')
            )->first(); 
            
        $item['other_expenses'] = Expense::where('expense_category_id', '!=', '3')->whereBetween('date', array($request->from, $request->to))
            ->select(
                DB::raw('SUM(amount) AS sum')
            )->first();     
